...
/**
 * Feature-map test-summary reporter.
 *
 * Reads feature-map/feature-map.yml, matches each feature id (SIxxxx) to an
 * executed test by the ID token embedded in the test title (e.g. a title
 * "SI0011 — ..." matches feature id SI0011), then renders a Markdown report
 * with three tables:
 *   - Final Statistics   — totals, coverage, duration
 *   - Spec File Statistics — per-spec breakdown
 *   - Covered Scenarios  — one row per feature id (ID / Type / Title / Status / Duration)
 *
 * The report is appended to $GITHUB_STEP_SUMMARY when running in GitHub
 * Actions (so it shows on the workflow run page) and is always written to
 * playwright-report/feature-map-summary.md as a portable artifact.
 *
 * Registered as a reporter in playwright.config.ts.
 */
...
/** Minimal YAML reader for the fixed `features:` list structure — no deps. */
...
/** Pulls the feature id (two letters + four digits) out of a test title. */
...
export default class FeatureMapReporter implements Reporter {
  ...
  onBegin(_config: FullConfig, suite: Suite): void {
    ...
    try {
      this.features = parseFeatureMap(fmPath);
    } catch (e) {
      console.warn(
        `[feature-map] could not read ${fmPath}: ${(e as Error).message}`,
      );
    }
  }
  ...
  onEnd(_result: FullResult): void {
    try {
      ...
      console.log(
        '[feature-map] summary written to playwright-report/feature-map-summary.md',
      );
    } catch (e) {
  ...
  private buildReport(): string {
    ...
    const scenarios: Scenario[] = this.features
      .map((f) => {
    ...
    const avg = total
      ? recs.reduce((a, r) => a + r.duration, 0) / total
      : 0;
    ...
    // Spec File Statistics — one row per spec file, drawn from both executed
    // tests and feature-map entries (so a spec with zero matched tests still
    // surfaces its Not Covered count).
    const files = [
      ...new Set([
        ...recs.map((r) => r.file),
        ...this.features.map((f) => f.spec),
      ]),
    ]
      .filter(Boolean)
      .sort();
    ...
    const specRows = files.map((file) => {
      ...
      const nc = scenarios.filter(
        (s) => s.status === 'notcovered' && s.spec === file,
      ).length;
      ...
      return [
        ...
        String(nc),
        fmtDuration(time),
        fr.length ? fmtDuration(time / fr.length) : '-',
      ];
    });

    const specStats = table(
      [
        'Spec File',
        'Total',
        'Passed ✅',
    ...
    const scenarioStats = table(
      ['ID', 'Type', 'Title', 'Status', 'Duration'],
      scenarioRows,
    );
    ...
    const unmappedNote = unmapped.length
      ? `\n> ⚠️ ${unmapped.length} executed test(s) have no feature-map id — give the test a "SIxxxx — " title prefix or add the id to feature-map/feature-map.yml.\n`
      : '';
